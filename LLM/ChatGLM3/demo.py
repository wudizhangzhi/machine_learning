...
TOKENIZER_PATH = MODEL_PATH = Path("D:\github\ml\Model\ChatGLM-6B-Int4")
...
if "cuda" in DEVICE:  # AMD, NVIDIA GPU can use Half Precision
    model = (
        AutoModel.from_pretrained(MODEL_PATH, trust_remote_code=True).to(DEVICE).eval()
    )
...
demo.queue().launch(
    share=False, server_name="127.0.0.1", server_port=8501, inbrowser=True
)
